...
QUARTER_FY_FIRST = re.compile(r"FY\s*(\d{2})\s*Q\s*([1-4])", re.IGNORECASE)
QUARTER_Q_FIRST = re.compile(r"Q\s*([1-4])\s*FY\s*(\d{2})", re.IGNORECASE)
...
def parse_ticker_and_quarter(pdf_path: Path) -> tuple[str, str]:
    ...
    filename prefix; quarter from an FYnnQn or QnFYnn token in the filename,
    normalized to FYnnQn (accepts either order and any case)."""
    ticker = pdf_path.parent.name if pdf_path.parent.name not in ("transcripts", "") else pdf_path.stem.split("_")[0]
    ticker = ticker.upper()
    stem = pdf_path.stem
    m = QUARTER_FY_FIRST.search(stem)
    if m:
        quarter = f"FY{m.group(1)}Q{m.group(2)}"
    else:
        m = QUARTER_Q_FIRST.search(stem)
        quarter = f"FY{m.group(2)}Q{m.group(1)}" if m else "UNKNOWN"
...
def build_dashboard_bundle() -> int:
    """Regenerate site/ledger.js from every VERIFIED promise file, so the static
    dashboard reads all promise JSONs offline (file://, no server). Only docs that
    have passed verification (pass_rate is not None) are included, and those docs
    hold verified promises only — nothing unverified reaches the dashboard."""
    companies = load_companies()
    docs = []
    for jf in sorted(PROMISES.glob("*.json")):
        doc = json.loads(jf.read_text(encoding="utf-8"))
        if doc.get("pass_rate") is None:
            continue  # not verified yet
        comp = companies.get(doc.get("ticker", ""), {})
        docs.append({
            "ticker": doc.get("ticker"),
            "company": doc.get("company"),
            "sector": comp.get("sector", ""),
            "ir_url": comp.get("ir_url", ""),
            "quarter_of_call": doc.get("quarter_of_call"),
    ...
    (SITE / "ledger.js").write_text(
        f"// Generated by pipeline.py render — do not edit by hand.\n"
        f"window.LEDGER = {payload};\n"
        f"window.LEDGER_GENERATED = {json.dumps(_dt.date.today().isoformat())};\n",
        encoding="utf-8",
    )
    return len(docs)
...
def cmd_render(json_arg: str) -> None:
    ...
    n = build_dashboard_bundle()
    print(f"render: refreshed dashboard bundle site/ledger.js ({n} verified transcript(s))")
