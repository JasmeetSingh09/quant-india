...
# Broad ~Nifty-200 liquid universe. Momentum shows NO edge in the 40-name
# large-cap list above but a large, significant edge here (see RESEARCH_momentum.md):
# cross-sectional momentum needs the dispersion that only breadth (mid-caps) provides.
# NOTE: uses today's constituents → survivorship bias inflates the magnitude.
BROAD_UNIVERSE = [t + ".NS" for t in (
 "RELIANCE TCS HDFCBANK INFY ICICIBANK HINDUNILVR ITC SBIN BHARTIARTL KOTAKBANK LT "
 "BAJFINANCE AXISBANK ASIANPAINT MARUTI HCLTECH SUNPHARMA TITAN ULTRACEMCO WIPRO "
 "NESTLEIND ONGC NTPC POWERGRID TATAMOTORS M&M TATASTEEL JSWSTEEL ADANIENT ADANIPORTS "
 "COALINDIA BAJAJFINSV TECHM GRASIM HDFCLIFE SBILIFE DRREDDY CIPLA DIVISLAB EICHERMOT "
 "BRITANNIA HEROMOTOCO BPCL INDUSINDBK BAJAJ-AUTO APOLLOHOSP HINDALCO TATACONSUM UPL IOC "
 "VEDL GAIL DABUR GODREJCP MARICO PIDILITIND SIEMENS DLF AMBUJACEM BANKBARODA PNB CANBK "
 "IDFCFIRSTB FEDERALBNK BANDHANBNK AUBANK CHOLAFIN SHRIRAMFIN MUTHOOTFIN ICICIPRULI "
 "ICICIGI HDFCAMC SBICARD BAJAJHLDNG LTIM LTTS MPHASIS PERSISTENT COFORGE OFSS TATAELXSI "
 "NAUKRI PAYTM ZOMATO POLICYBZR NYKAA INDUSTOWER IDEA TATACOMM TATAPOWER ADANIGREEN "
 "ADANIPOWER ADANIENSOL NHPC SJVN POWERINDIA BHEL BEL HAL BDL MAZDOCK COCHINSHIP IRCTC "
 "IRFC RVNL CONCOR IRB GMRINFRA LICI LICHSGFIN PFC RECLTD BIOCON LUPIN AUROPHARMA "
 "TORNTPHARM ALKEM ZYDUSLIFE GLENMARK IPCALAB LAURUSLABS GLAND SYNGENE MANKIND ABBOTINDIA "
 "PEL FORTIS MAXHEALTH LALPATHLAB METROPOLIS JUBLFOOD TRENT DMART PAGEIND BATAINDIA RELAXO "
 "VOLTAS HAVELLS CROMPTON WHIRLPOOL DIXON AMBER BLUESTARCO POLYCAB KEI APLAPOLLO JINDALSTEL "
 "SAIL NMDC NATIONALUM JSWENERGY TATACHEM SRF AARTIIND DEEPAKNTR NAVINFLUOR PIIND COROMANDEL "
 "CHAMBLFERT UBL COLPAL EMAMILTD HONAUT 3MINDIA CUMMINSIND ABB THERMAX BOSCHLTD MOTHERSON "
 "BALKRISIND MRF APOLLOTYRE BHARATFORG ASHOKLEY TVSMOTOR ESCORTS SONACOMS OBEROIRLTY "
 "GODREJPROP PRESTIGE PHOENIXLTD LODHA INDHOTEL IGL MGL GUJGASLTD PETRONET OIL HINDPETRO "
 "BERGEPAINT KANSAINER ACC SHREECEM DALBHARAT RAMCOCEM JKCEMENT INDIGO DELHIVERY ASTRAL "
 "SUPREMEIND BALRAMCHIN CGPOWER"
).split()]
